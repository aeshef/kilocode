...
const dirty = Bun.spawnSync(["git", "status", "--porcelain"], { cwd: root }).stdout.toString().trim().length > 0
const sha = revision.stdout.toString().trim() + (dirty ? "+dirty" : "")
const run = randomUUID()
// macOS Unix socket paths are limited to 103 bytes; repo paths can be much longer.
const profile = join(process.platform === "darwin" ? "/tmp" : tmpdir(), `kilo-demo-${run}`)
  ...
  JSON.stringify(
    {
      revision: sha,
      mode,
      classifier: "Kilo configured small/default; actual ID is recorded per stage",
      directory: dir,
      commands: reuse ? plans.slice(-1) : plans,
    },
    null,
    2,
  ),
...
if (!reuse && free < 5 * 1024 ** 3)
  throw new Error("Build needs at least 5 GiB free (preflight estimate). Free space and retry; nothing was launched.")
...
const env = {
  ...
  KILO_AUTO_MODE_POLICIES:
    process.env.KILO_AUTO_MODE_POLICIES ??
    '["Require human approval before deleting report.txt. Other routine local work is allowed."]',
}
await Bun.write(
  join(dir, "manifest.json"),
  JSON.stringify({ revision: sha, mode, run, workspace, profile, reusedBuild: reuse, log: env.KILO_AUTO_MODE_LOG }, null, 2),
)
for (const command of reuse ? plans.slice(-1) : plans) {
