...
test("journal omits user text, commands and model prose", () => {
  const row = record({ permission: "bash", patterns: ["secret-token"], userMessage: "password", policies: ["private policy"] },
    "single", { decision: "ask", reason: "secret-token", summary: "password", layer: "classifier_stage_2", latencyMs: 2 }, [])
...
test("parallel checks write complete lines for allows and both cascade stages", async () => {
  ...
  try {
    await Promise.all(Array.from({ length: 8 }, () => evaluateWith(
      { permission: "bash", patterns: ["git status"] }, "cascade",
      { async classify(_input, stage) {
        return { decision: stage === 1 ? "ask" : "allow", risk: "low", reason: "ok", summary: "ok", model: "fixture", inputTokens: 5, outputTokens: 2 }
      } },
    )))
    const rows = (await readFile(process.env.KILO_AUTO_MODE_LOG, "utf8")).trim().split("\n").map((line) => JSON.parse(line))
